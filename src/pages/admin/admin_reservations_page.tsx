...
interface StoredFilters {
  ...
  sortBy: 'createdAt' | 'updatedAt' | 'reservationDate';
  sortOrder: 'asc' | 'desc';
}
...
export const AdminReservationsPage: React.FC = () => {
  ...
  const { reservations, pagination, isLoading, error } = useAdminReservationsList({
    page: currentPage,
    limit: itemsPerPage,
    includeDeleted,
    search: searchQuery,
  ...
  // Get sort by label
  const getSortByLabel = (value: 'createdAt' | 'updatedAt' | 'reservationDate'): string => {
    if (value === 'createdAt') return 'Created Date';
    if (value === 'updatedAt') return 'Updated Date';
                ...
                {showSortBy && (
                  <div className="absolute top-full left-0 mt-1 w-48 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-lg shadow-lg z-50 p-2">
                    <div className="space-y-1">
                      {(['createdAt', 'updatedAt', 'reservationDate'] as const).map((option) => (
                        <button
                          key={option}
                          onClick={() => {
