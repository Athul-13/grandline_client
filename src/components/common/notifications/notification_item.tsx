...
export const NotificationItem: React.FC<NotificationItemProps> = ({
  ...
  return (
    <button
      onClick={handleClick}
      className={cn(
        'w-full text-left px-4 py-3 hover:bg-[var(--color-bg-hover)] transition-colors',
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            ...
            {!notification.isRead && (
              <div className="flex-shrink-0 w-2 h-2 rounded-full bg-[var(--color-primary)] mt-1.5" />
            )}
          </div>
          <p className="text-sm text-[var(--color-text-primary)] line-clamp-2">
            {notification.message}
          </p>
          <p className="text-xs text-[var(--color-text-secondary)] mt-1">{timeAgo}</p>
        </div>
      </div>
    </button>
  );
