 ...
 */
export const PassengersBentoCard: React.FC<PassengersBentoCardProps> = ({ reservationDetails }) => {
  const passengers = reservationDetails.passengers || [];
    ...
    <div className="bg-[var(--color-bg-card)] rounded-lg shadow-sm border border-[var(--color-border)] p-4 flex flex-col h-full min-h-[250px]">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="ml-auto text-sm text-[var(--color-text-secondary)]">
          {passengers.length > 0 ? passengers.length : reservationDetails.passengerCount || 0}
        </span>
      </div>
