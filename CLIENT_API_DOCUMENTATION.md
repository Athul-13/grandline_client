...
### 1. SENT (Single Tick)
- **Status**: `'sent'`
- **Meaning**: Message has been saved to the database
- **When**: Immediately after message is sent
- **Display**: Single gray tick

### 2. DELIVERED (Double Gray Tick)
- **Status**: `'delivered'`
- **Meaning**: Recipient is online and connected via Socket.io
- **When**: Recipient joins the chat room or is already in the chat room
- **Display**: Double gray tick
- **Event**: `message-delivered` is emitted

### 3. READ (Double Blue Tick)
- **Status**: `'read'`
- **Meaning**: Recipient is actively viewing the chat
- **When**: Recipient marks messages as read (via REST API or socket event)
...
socket.emit('send-message', { chatId: 'chat_789', content: 'Hello' });
// Status: SENT (single tick)

// 2. Recipient is online
socket.on('message-delivered', ({ messageId }) => {
  // Update message status to DELIVERED (double gray tick)
});
